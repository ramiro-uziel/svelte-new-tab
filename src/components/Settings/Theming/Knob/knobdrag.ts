...
function hasCapture(pointerId: number, elem: HTMLElement) {
	return elem.hasPointerCapture(pointerId) || document.pointerLockElement === elem;
}
...
function pointerlock(elem: HTMLElement) {
	function enterLock(event: MouseEvent) {
		if (event.button !== 2) elem.requestPointerLock();
	}
	function exitLock() {
...
function pointercapture(elem: HTMLElement) {
	function pointerDown(event: PointerEvent) {
		if (event.button !== 2) elem.setPointerCapture(event.pointerId);
	}
